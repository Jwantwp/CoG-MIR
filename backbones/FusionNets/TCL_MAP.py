...
from .Enhance import ProgressiveEnhancerWithAuxToken
...
class MAP(BertPreTrainedModel):
    def __init__(self, config, args):
        ...
        self.progressive_enhancer = ProgressiveEnhancerWithAuxToken(args)
    ...
    Return:
        ...
        X_enhanced = self.progressive_enhancer(fused_embedding, all_pooled)
        # X_enhanced = torch.cat([fused_embedding, all_pooled[0].unsqueeze(1)], dim=1)  # (16, 37, 768)
        
        enhance_logits = enhance_llm['logits']  # (16, 1000)
        
        # print("==========TCL-MAP的融合后的维度为：============",fused_embedding.shape)  #=========TCL-MAP的融合后的维度为：============ torch.Size([16, 43, 768]) 9_30
        ...
        return outputs, generated_ctx, enhance_logits
...
class MAP_Model(BertPreTrainedModel):
    ...
    Returns:
        ...
        outputs, generated_ctx,enhance_logits = self.bert(
            input_ids,
            visual,
            acoustic,
            enhance_feats,
        ...
        return outputs, pooled_output, condition, generated_ctx, enhance_logits
...
class TCL_MAP(nn.Module):
    ...
    def forward(self, text_feats, video_feats, audio_feats, enhance_feats,cons_text_feats, condition_idx):
        video_feats, audio_feats = video_feats.float(), audio_feats.float()

        # process normal sample
        outputs, pooled_output, condition, generated_ctx,enhance_logits = self.model(
            text = text_feats,
            visual = video_feats,
        ...
        return outputs[0]+enhance_logits, pooled_output, condition.mean(dim=1), cons_condition.mean(dim=1)
