...
from .Enhance import Enhance_LLM
from .Enhance import ProgressiveEnhancer
...
class MISA(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.max_length = args.text_seq_len
        ...
        self.fusion.add_module('fusion_layer_1', nn.Linear(in_features=args.hidden_size * 6+768, out_features=args.hidden_size * 4))
        ...
        self.fusion.add_module('fusion_layer_3', nn.Linear(in_features=args.hidden_size * 4, out_features= self.output_dim))
        ...
        self.enhance_models = Enhance_LLM(args)
        self.progressive_enhancer = ProgressiveEnhancer(args)
    ...
    def forward(self, text_feats, video_feats, audio_feats,enhance_feats):
        ...
        video_feats = pad(video_feats, (0, 0, 0, max_seq_len - video_feats.shape[1]), value=0)[:,:max_seq_len,:]
        ...
        audio_feats = pad(audio_feats, (0, 0, 0, max_seq_len - audio_feats.shape[1]), value=0)[:,:max_seq_len,:]
        ...
        h = torch.cat((h[0], h[1], h[2], h[3], h[4], h[5]), dim=1) # (6, batch_size, hidden_size)
        
        self.args.main_dim= h.shape[1]  # 1536  9_30
        
        enhance_llm= self.enhance_models(enhance_feats)
        all_pooled = enhance_llm['all_layer_pooled']  # list of (16, 768)
        X_enhanced = self.progressive_enhancer(h, all_pooled)
        
        enhance_logits = enhance_llm['logits']  # (16, 1000)
        
        # print("=============",h.shape)  #torch.Size([16, 1536])  9_30
        
        # logits = self.fusion(h)
        
        logits_main = self.fusion(X_enhanced)
        logits = logits_main + enhance_logits
