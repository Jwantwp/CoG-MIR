...
class MULT(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.dropout_rate = args.enhance_dropout_rate
        self.activation = nn.ReLU()
        self.fusion = nn.Sequential()
        self.fusion.add_module('fusion_layer_1', nn.Linear(in_features=combined_dim+args.enhance_dst_feature_dims, out_features=args.enhance_dst_feature_dims))
        self.fusion.add_module('fusion_layer_1_dropout', nn.Dropout(self.dropout_rate))
        self.fusion.add_module('fusion_layer_1_activation', self.activation)
        self.fusion.add_module('fusion_layer_3', nn.Linear(in_features=args.enhance_dst_feature_dims, out_features= output_dim))

        # self.out_layer = nn.Linear(combined_dim+args.enhance_dst_feature_dims, output_dim)
    ...
    def forward(self, text_feats, video_feats, audio_feats, enhance_feats):
        ...
        # print("=======最后的融合维度为：=====",last_hs_proj.shape)#======最后的融合维度为：===== torch.Size([32, 1536])  9_30
        
        enhance_llm= self.enhance_models(enhance_feats)
        
        all_pooled = enhance_llm['all_layer_pooled']  # list of (16, 768)
        
        X_enhanced = self.progressive_enhancer(last_hs_proj, all_pooled)
        
        enhance_logits = enhance_llm['logits']
        
        
        # print("=======增强后的维度为：=====",X_enhanced.shape)
        # logits = self.out_layer(X_enhanced)  # [16,20]
        logits_main = self.fusion(X_enhanced)
        # logits_main=self.out_layer(X_enhanced)
